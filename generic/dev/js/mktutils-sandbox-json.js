...
function loadMarketingInformation() {
 ...
 window.marketing_tracker_values = [];
  var currentURL = document.location.href;
  currentURL.substring(0, 255);
  // Load each of our straightforward fields, and add them to our
  // form as hidden inputs.
  for (var i = 0; i < fields.length; i++) {
    var the_field = fields[i].substring(0, 255);
    var value = jQuery.cookie("mkt_" + the_field);
    if ((value != null) && (value != "")) {
      var the_value = decodeURIComponent(value).substring(0, 512);
      var input_element = jQuery("<input name='marketing_tracker[" + the_field + "]' type='hidden' />");
      input_element.val(the_value);
      jQuery('form').append(input_element);
      window.marketing_tracker_values.push({"key" : the_field, "value" : the_value, "source" : currentURL});
    }
  }
  //  Stringfy the array and put it in a hidden form field
  var marketing_tracker_values_string = JSON.stringify(window.marketing_tracker_values);
  var marketing_tracker_values_input = jQuery("<input name='marketing_tracker_values' id='marketing_tracker_values' type='hidden' />");
  marketing_tracker_values_input.val(marketing_tracker_values_string);
  jQuery('form').append(marketing_tracker_values_input);
...
// Added this to the global scope to be able to call this whenenver we need to update the marketing_tracker_values field
function updateMarketingTrackerValues(key, value, source) {
  window.marketing_tracker_values.push({"key" : key, "value" : value, "source" : source});
  //  Stringfy the array and put it in a hidden form field
  var marketing_tracker_values_string = JSON.stringify(window.marketing_tracker_values);
  var marketing_tracker_values_input = document.getElementById("marketing_tracker_values");
  marketing_tracker_values_input.value = marketing_tracker_values_string;
}
